...
def draw_pages(draw_criteria, pages, current_dir, remainder, whole_pages, args, log):
    """controls the creation of the body pages"""
    ...
    if draw_criteria == 'test':  # test area (unused)
        ...
        if whole_pages is False:  # this special 'if' creates the last (not completely filled) page. Clean up?
            page_draw(remainder, pages, current_dir, total_pages, args, log)
            pages -= 1
            print('|', end='')
        else:
            pass
        while pages > 0:
            page_draw(500, pages, current_dir, total_pages, args, log)
            pages -= 1
            page_counter += 1
            print('|', end='')
...
def page_draw(page_icon, pages, current_dir, total_pages, args, log):  # this routine creates individual pages
...
def death_get(data_source, args, log):  # retrieves the dataset from coronavirus.gov.uk
...
def front_matter(mode, current_dir, args, log):  # creates the front matter of the book.
    blanks = 2
...
def back_matter(pages, args, log):  # creates the back matter of the book.
    if pages % 2 == 0:
        blanks = 2
    else:
        blanks = 3
...
def cover_maker(current_dir, working_dir, args, log):  # makes the cover. Does not currently adjust for page count
...
def pdf_maker(today, args, log):  # creates the print-ready PDF
